...
**How to apply:** if the dialogue-loss detector flags `docs/episode-99.md` specifically, verify its content is placeholder text (not real dialogue), then `git checkout <last-good-commit> -- docs/episode-99.md` to restore it — the detector explicitly treats content present again at the tip as "self-healed by a later commit; not blocking." This is a recurring risk, not a one-time fix: the path can be re-orphaned by the next `test-rolling-sync-guard.ts` run.

**Fixed (task 1596, Sep 26 2026):** the git-tracking hazard described above is closed. `docs/episode-99.md` is now both listed in `.gitignore` and untracked from the git index (`git rm --cached`) — gitignoring alone does nothing for a path git already has in its index, both steps were required together. A leftover from an early-exit failure path (e.g. a `process.exit()` before `cleanup()` runs) is now invisible to `git status` and can no longer be swept into a commit via `git add -A`. `detect-episode-dialogue-loss.ts` no longer needs a manual restore commit for this specific path. This is separate from failure mode #3 just below (`test-chat-episode-hook-e2e.ts` Step 4/5) and failure mode #1 above (`test-rolling-sync-guard.ts` Pass 2/3's "DB stayed canonical but .md was not restored") — those are real, still-open bugs in the rolling-episode sync/restore direction, unaffected by this fix. Confirmed post-fix: a fresh full `test-all-consolidated-ci.sh` run (Sep 26 2026) showed `test-rolling-sync-guard.ts` passing cleanly (no Pass 2/3 failure this run) while `test-chat-episode-hook-e2e.ts` Step 4/5 still failed with the exact "Sentinel appears in DB record" symptom from failure mode #3 — the two `episode-sync` failure modes are confirmed independent of each other and of this fix.
