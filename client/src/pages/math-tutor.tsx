...
import { ArrowLeft, Calculator, BookOpen, Library } from "lucide-react";
...
export default function MathTutor() {
  ...
  const [conversationId, setConversationId] = useState<string | null>(() => {
    return sessionStorage.getItem('mathConversationId') || null;
  });
  ...
  useEffect(() => {
    if (conversationId) {
      sessionStorage.setItem('mathConversationId', conversationId);
      return;
    }
    ...
    apiRequest("POST", "/api/conversations", {
      ...
      .catch(err => {
        console.error("[Math] Failed to create conversation:", err);
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <Button
            size="icon"
            variant="ghost"
            onClick={() => setLocation('/reading-library?subject=calculus-vol1')}
            title="Reading Library"
            data-testid="button-reading-library-math"
          >
          ...
          <Button
            ...
            className={showReadingPanel ? "bg-blue-600 text-white" : ""}
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className={`flex-1 overflow-hidden transition-all duration-300 ${showReadingPanel && !isMobile ? 'min-w-0' : ''}`}>
          <StreamingVoiceChat
            ...
            targetLanguageOverride="math"
            homeRoute="/math-tutor"
          />
        </div>
