...
function TutorObservationsBubble({
  ...
  const x = isMobile ? -30 : -100; // Push left outside container for overlap effect
  const y = isMobile ? 10 : 0; // Upper area
  ...
  return (
    <>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 35 }}>
        ...
        {!isExpanded && (
          <path
            d={`M ${x + collapsedWidth - 15} ${y + collapsedHeight / 2 + 10} Q ${x + collapsedWidth + 80} ${y + 50} ${centerX - 45} ${centerY - 65}`}
            stroke={accentColor}
            strokeWidth="2.5"
            fill="none"
            strokeLinecap="round"
...
export function SyllabusMindMap({ classId, language: languageProp, className, syllabusOverview, mode = 'emergent' }: SyllabusMindMapProps) {
  ...
  return (
    <div className={`${className} ${isMobileView ? 'overflow-hidden' : 'overflow-visible'}`} data-testid="syllabus-mind-map">
      ...
      <div className={`relative ${isMobileView ? '-mt-28' : '-mt-60'}`} data-testid="activity-inputs-container">
