...
**2026-07-01 — `server/services/gemini-live-session.ts` — GL tail audio artifacts ("ok" / "hey" appended at end of responses) — MEDIUM**
After Daniela's main audio completes, a brief filler word (e.g. "ok", "hey") is occasionally heard. Root cause: GL emits a micro sub-turn after `generationComplete` to "fill" any remaining audio budget. The `transcriptClosed` gate suppresses transcript accumulation after `generationComplete`, but audio chunks from this tail sub-turn still reach the client. Fix candidates: (a) ignore all audio arriving after `generationComplete` fires; (b) add a post-`generationComplete` audio gate on the client (suppress any `audio_chunk` arriving <200ms after `response_complete`); (c) configure GL session with `audioContextDuration` or `mediaResolution` settings to prevent tail generation. Needs server log correlation: check if a `turnComplete` fires immediately after `generationComplete` with zero `outputTranscription` — that's the tail turn.

**2026-07-01 — `client/src/components/VoiceChatViewManager.tsx` / ImmersiveTutor — Japanese text pill visible near Live/History badges in English (Cindy) session — LOW-MEDIUM**
Screenshot shows "そうですね" in a small pill/chip near the History badge (`absolute top-5 right-10`) during a Cindy English session. Likely causes: (a) subtitle state not reset between sessions — if subtitle mode was 'target' from a prior Japanese session, the last sentence's text persists until new audio arrives; (b) `CollaborationIndicator` in ImmersiveTutor rendering the tutor's last utterance near the top-right area; (c) GL `response_text` display not found in client code — this would be silently dropped. Needs the original screenshot to narrow down the exact element. Fix path: ensure subtitle state / `currentSentenceText` is cleared to '' when a new GL session starts (`start()` call or `gl_reconnected` event on client).

**2026-07-01 — `server/services/daniela-function-registry.ts` / GL dispatcher — Sports broadcast: Daniela recites scores verbally but doesn't display scoreboard widget — MEDIUM**
When Daniela runs a sports broadcast (`get_broadcast_data` type='sports'), she narrates the scores but doesn't automatically trigger a scoreboard widget. Expected: she should call `widget_board` or a dedicated scoreboard tool to display team names + scores visually. Fix: add example in the broadcast tool description showing that sports broadcasts should pair `get_broadcast_data` with a visual scoreboard display; or add a dedicated `SHOW_SCOREBOARD` tool handler that renders structured match data. The `broadcast-data-service.ts` already fetches sports data — the gap is the visual display layer.
