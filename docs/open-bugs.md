...
**2026-07-10 — `server/services/gemini-live-session.ts` `actflSilenceDurationMs` — Daniela's turn submitted prematurely on a mid-sentence pause — MITIGATED, root cause not fully isolated**
Live controlled experiment with David (transcript: `attached_assets/Pasted-Uh-it-does-make-sense...1783711675085.txt`): David paused mid-sentence for a few seconds and his turn was cut off/submitted early, letting Daniela jump in. The GL turn-detection config (`endOfSpeechSensitivity: LOW`, dynamic `silenceDurationMs` per ACTFL level) was already set to its most lenient documented values (novice: 5000ms, unassessed default: 4000ms) — on paper generous enough to cover the observed pause length, so the numbers alone don't explain the cutoff. Did not confirm whether `session.studentActflLevel` is reliably set *before* `createGeminiLiveSession` builds the config for every code path (greeting-flow vs. reconnect-flow construction sites in `unified-ws-handler.ts` are separate from where `studentActflLevel` gets assigned in `streaming-voice-orchestrator.ts`) — an ordering bug there would silently fall through to the `4000ms` default even for assessed students, but that alone still doesn't explain a cutoff on a level that should get 5000-6000ms. Applied an immediate mitigation: bumped every ACTFL tier up ~1000-1200ms (novice 5000→6000, unassessed default 4000→5000, etc.) while the ordering question and any client-side/idle-timer interaction is investigated further. Typecheck clean, deployed via dev-workflow auto-restart July 10. Follow-up: instrument a log line at GL config-build time printing the actual `studentActflLevel` + resolved `silenceDurationMs` used per session, to confirm the right number is actually reaching the API.

**2026-07-10 — `server/unified-ws-handler.ts` reconnect grace claim / `client/src/lib/streamingVoiceClient.ts` reconnect flow — reconnect grace period expired UNCLAIMED after a hard drop, session resumed cold instead of continuing — INVESTIGATING (escalated from the chronic-drop entry below)**
