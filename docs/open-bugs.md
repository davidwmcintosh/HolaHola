...
Daniela starts every new GL session at a default gear with no memory of where the previous session ended. A student who closed in Gear 1 (highly frustrated/struggling) will be greeted by an uncalibrated Daniela next time. Fix: at session init, query `pedagogical_snapshots` for most recent row by userId (ordered by `created_at` DESC, limit 1) and inject the gear + fluencyMomentary into the classroom environment or pre-session synthesis. Gemini R2 flagged as "Day 2 priority — acceptable to ship without it."
