...
**FIXED 2026-06-30** — `sendStreamingChunk` now buffers up to 100 chunks (~1s of audio) when the WebSocket is not ready. `_flushPendingChunks()` replays the buffer immediately after reconnect + `set_input_mode` restoration. Fresh (non-reconnect) `startSession` calls always clear the buffer so stale chunks never bleed into a new session.
