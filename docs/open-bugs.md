...
**2026-07-25 — GL greeting + mid-sentence cut-off ("It's…" lost; "it's —" lost) — FIXED July 25 (4th fix)**
Two cut-offs reported after Fix 3. Root cause (both): `flushTranscripts()` was called IMMEDIATELY at `generationComplete`, before the 200ms debounce seal had fired. This caused two cascading failures: (1) `totalSentences = currentSentenceIndex` was off-by-one — the in-progress sentence wasn't counted yet; (2) the per-response reset inside `flushTranscripts()` (`firstAudioSentThisTurn = false`, `processingPendingSentThisTurn = false`) fired prematurely. When in-flight audio chunks arrived after the flush but before the seal, the `!firstAudioSentThisTurn && !wasGreetingPhase` check at line ~1993 was now TRUE → `processing_pending` fired → `resetForNewTurn()` was called on the audio player → all buffered audio for the current sentence was wiped. Greeting symptom: "It's" played, then avatar switch to thinking, then "I was thinking…" — all of "good to have you here, David. You know," was lost between them. Conversation symptom: "it's not about probability then; it's —" cut mid-clause. Fix: moved `flushTranscripts()` from the immediate `generationComplete` call INTO the 200ms debounce callback (after `sealCurrentAudioSubturn`). Applied the same change to the extended debounce (when audio arrives after generationComplete resets the timer). Watchdog path already calls `flushTranscripts()` post-seal (unchanged). Typecheck clean July 25.
Location: `server/services/gemini-live-session.ts` — `generationComplete` handler (~line 2886) + audio chunk debounce extension (~line 1847) — Severity: HIGH (audio lost mid-sentence, avatar reset during greeting)
...
**2026-07-20 — GL audio never refreshed orchestrator `lastActivityTime` — FIXED July 20**
In GL mode, the orchestrator's `lastActivityTime` was only updated via the STT path (greeting/response handlers). GL binary audio never called `resetIdleTimeoutForSession`. The session monitor's stall check compares `Date.now() - lastActivityTime` against `SESSION_STALE_THRESHOLD_MS` (30s) — since GL sessions can run without any STT activity for minutes, the monitor was firing `⏸️ Stalled session` every 30 seconds the entire time David was in a live GL session and posting those alerts to Team Room. Fix 1: GL binary audio handler in `server/unified-ws-handler.ts` now calls `orchestrator.resetIdleTimeoutForSession(session.id)` on every audio chunk. Fix 2: session monitor in `server/services/session-monitor.ts` now skips GL sessions entirely (geminiLiveToolsOnly === true) since they manage their own idle timeout independently. Typecheck clean, deployed July 20.

**2026-07-20 — GL WebSocket drops in production → Daniela goes back / repeats herself — FIXED July 20**
When the GL WebSocket to Gemini dropped in production, the reconnect injection in `streaming-voice-orchestrator.ts` was an explicit verbal announcement ("Oh, we got cut off!"). Changed to silent state-as-knowledge prose: "Your thoughts are currently focused on the following exchange: [history]. Maintain the flow of the lesson seamlessly. Pick up exactly where the conversation left off." History preview slice increased 80→250 chars. Purely positive framing (no pink-elephant "don't mention X" constraints). Gemini pre-flight + post-review approved. Typecheck clean July 20. Note: underlying connection instability is still a production environment concern; this fix handles the symptom (jarring repetition) without claiming to prevent drops.

**2026-07-20 — Neon WebSocket pool drops in production → recall/introspect tools fail → Daniela says "I don't know who Alden is" — FIXED July 20**
`processUnifiedRecall` in `server/services/native-fc-handlers.ts` — all six arms hardened. Arms 3 (Express Lane), 4 hydration, 5 (conversation_memories), and 6 (image memory) switched from `getSharedDb()` (WebSocket pool) to `getMonitoringDb()` (HTTP transport — no WebSocket, no pool exhaustion, SELECT-only). Arms 1 and 2 (pgvector — can't switch to HTTP) wrapped with 1500ms `Promise.race` timeout so pool exhaustion causes them to fail-fast and return null instead of hanging. Arm 4 `semanticSearch()` also gated with 1500ms timeout. Arm 4 hydration loop converted from sequential `for` loop to parallel `Promise.all` with pre-dedup pass (eliminates `seenConvMemIds` race condition that would have existed in a naive parallelization). Gemini pre-flight + two-round post-review, unconditional APPROVED. Typecheck clean July 20.

**2026-07-20 — Legacy tools `call_support`/`call_assistant` removed from registry — FIXED July 20**
Two legacy dispatcher entries (`call_support`, `call_assistant`) were removed from `admin_tools` dispatcher. `escalate_to_support` description hardened to prevent misfires.
