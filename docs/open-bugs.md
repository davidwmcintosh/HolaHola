...
**2026-06-16 — `server/services/native-fc-handlers.ts:2201` — SET_EMOTION widget not appearing in Studio board — MEDIUM**
When Daniela calls `SET_EMOTION` (standalone, no active immersive scene), the emotion widget fails to render. The handler emits a `whiteboard_update` with a `scene_canvas` item containing `buildFullSceneCanvasData(session.sceneCanvas, 'set_emotion')`. If `session.sceneCanvas` is null/undefined at that moment the update may build an empty scene object. On the client, WhiteboardPanel may then discard or mishandle a scene_canvas item with no environment set. Symptom: gray rectangle placeholder appears but emotion face never shows. Fix candidate: check `session.sceneCanvas` is initialized before SET_EMOTION; or emit emotion as a standalone widget item type rather than embedding it in scene_canvas.

**2026-06-16 — `server/services/native-fc-handlers.ts:850` — CLEAR function triggers immersive-scene black screen when sceneCanvas is partially initialized — MEDIUM**
When the user asks Daniela to "clear the board," the CLEAR handler sends `items: [{ type: 'clear' }]` and then, if `session.sceneCanvas` exists, re-injects the scene with `canvasAction: 'open_scene'`. David observed the full Studio board going black (and appearing to enter fullscreen) even though no immersive scene was intentionally active. Likely cause: `session.sceneCanvas` has a stale/partial entry (e.g., `environment` set but `environmentImageUrl` empty) from a previous widget call (SET_EMOTION or similar), so the re-injection fires an `open_scene` with no image → black screen. Fix candidate: only restore the scene in CLEAR if `session.sceneCanvas.environmentImageUrl` is non-empty; otherwise skip the scene restore entirely.

**2026-06-16 — `server/services/native-fc-handlers.ts` — Widget commands fire all widgets when only one was requested — MEDIUM**
When Daniela was asked to "put up just the clock widget," all three (temperature, time, weather) appeared. Likely a state management issue: the pending/deferred `whiteboard_update` messages from an earlier multi-widget call were buffered (`session.pendingWhiteboardUpdates`) and then all flushed together when the next call was processed. Fix candidate: audit `pendingWhiteboardUpdates` flush logic — only flush the messages queued for the current function call, not all accumulated pending updates from prior calls.
