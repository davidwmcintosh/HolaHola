...
**2026-07-20 — GL WebSocket drops in production → Daniela goes back / repeats herself — OPEN**
When the GL WebSocket to Gemini drops in production (network instability, not timer-related), the client reconnects and the `isResumed && hasConversationHistory` path injects a "we got cut off" framing plus the last 4 messages as context. This causes Daniela to explicitly reference the cutoff and recap what was said, which sounds like she's going backwards. The disconnect itself is visible in production logs (`[GeminiLive] Session closed — code: N, reason: ...`). Root cause is Neon/Gemini connection instability under production load — not fixable without investigating the production environment's WebSocket connection pressure. Mitigation: the stall-monitor false alarm fix (above) removes the 30s spam that was unrelated noise during these events. Follow-up: consider softening the "we got cut off" injection to be less explicit ("continue from where you left off" vs. "I notice we got cut off") to reduce the jarring repetition effect.

**2026-07-20 — Neon WebSocket pool drops in production → recall/introspect tools fail → Daniela says "I don't know who Alden is" — OPEN**
In production, the Neon serverless WebSocket pool drops under load, causing ALL in-flight DB queries to fail simultaneously. Multiple background workers (editor_insights, agent_notes, brain_events, memory_embeddings, sophia_incidents, collaboration_messages, team_rooms) all fail at the same time, visible in production logs as `ErrorEvent { [Symbol(kTarget)]: WebSocket }` errors. When this happens mid-session, Daniela's `recall`/`introspect` tool calls return empty results — she gets zero context from all 6 search arms in `processUnifiedRecall`. Result: she can't find memories about Alden, David's family, or session history. Mitigation present: the pool already has a keepalive heartbeat every 3 minutes (`SELECT 1`). Gap: read-only background workers (session monitor, workers) compete with live GL tool calls for the 25-connection pool. No retry logic on `processUnifiedRecall` arms. Follow-up: consider switching `processUnifiedRecall` arms to use `getMonitoringDb()` (HTTP transport — no WebSocket, no pool exhaustion) for the Express Lane search (Arm 3) and structured memory (Arm 1) where pgvector is not needed.
