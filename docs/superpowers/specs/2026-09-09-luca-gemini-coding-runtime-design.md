...
Distinct-hat identity is necessary but not sufficient: the verifying registration must also be explicitly designated a standing verifier through the operator-only `designateStandingCoordinationVerifier` maintenance operation, restricted to the `luca-replit`/`luca-claude-code` actor identities and to a registration actually owned by the calling actor. A registration minted by ordinary per-task provisioning (e.g. the antigravity bootstrap flow) is never a standing verifier by default; `verify()` rejects any principal lacking the designation with `verifier_registration_not_standing`, independent of and prior to the assigner/self/actor checks above. This keeps "who may verify at all" an explicit operator decision rather than an emergent property of which credentials happen to exist.
...
No new actor value, static-token binding, runtime registration table, credential table, credential renewal path, or runtime lease table is added. `coordination_runtime_registrations` does gain an additive, operator-only standing-verifier designation: a `standing_verifier` boolean plus `standing_verifier_designated_at`/`standing_verifier_designated_by` provenance columns, with a check constraint enforcing the two are set together (both null while false, both set the moment an operator flips it true). Default `false` on every existing and newly-provisioned row; only `designateStandingCoordinationVerifier` may set it.
