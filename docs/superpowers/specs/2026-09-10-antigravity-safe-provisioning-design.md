...
- required starting commit: `5a2fbbf813597b93ffc03fbf2fd9977f4915e17f`.

The worktree must be clean at that exact commit before preparation begins.
...
1. Fast-forward the clean Windows worktree to the required starting commit.
2. Create and review the exact task #1448 artifact in `.local/tasks/task-1448.md`.
3. Generate or reuse task #1448's local ownership key in that worktree.
4. Create the bootstrap in the runtime-specific 1Password item.
5. Run the Windows preparation command under 1Password injection and export only the public provisioning bundle.
6. Replit validates the bundle and submits the ownership challenge.
7. The founder approves the challenge, including the exact task digest, actor, and ownership-key binding.
8. Replit registers the runtime with the bootstrap digest and creates its coding-runtime profile from the same reviewed bundle.
9. Antigravity starts under 1Password injection and exchanges the original bootstrap for a short-lived broker credential.
10. Antigravity signs the server nonce with the Windows-held ownership private key and proves the active founder-approved receipt.
11. Only after proof succeeds may the runtime fetch, consume, claim, or execute task #1448.
