...
3. **Ghost Image failure path:** If DALL-E fails silently, `sendRealtimeInput({ text })` was originally used — but both Gemini audits flagged it as the PCM audio channel (text treated as student speech). Fixed: no injection. Receipt framing ("should appear... do not describe visual details until image arrives in vision feed") is the guardrail — no image in vision feed = no description.
...
**GL 3.5 migration — model does not exist yet:**
Both Gemini audits recommended `gemini-3.5-flash-live-preview`. Verified via models API: **404 NOT FOUND**. Same hallucination pattern as `session_update` (see docs/gemini-live-session-update-research.md). Available live models: `gemini-3.1-flash-live-preview` (current), `gemini-2.5-flash-native-audio-*` (native audio variants). Watch for `gemini-3.5-flash-live-preview` — when it ships, the comparison matrix from today's audit is already written up.
...
**HayUnitContent additions (`client/src/data/madrigal-unit-content.ts`):**
- Japanese: Added `JAPANESE_MASU_PATTERN` (`the verb pattern japanese` — ます形 4-form polite frame). Total: 13 entries in HAY_UNITS.
- Korean: Added `KOREAN_TENSE_PATTERN` (`the verb pattern korean` — 아/어요 틀, 3-tense frame) + `KOREAN_WOULD_LIKE` (`i would like korean` — 고 싶은데요 / 고 싶었어요). Total: 13 entries in HAY_UNITS.
- Mandarin: Added `MANDARIN_ASPECT_PATTERN` (`the verb pattern mandarin` — 了/过/着 aspect markers) + `MANDARIN_XIANG_YAO` (`i would like mandarin` — 想要 / 能不能 politeness ladder). Total: 13 entries in HAY_UNITS.
- English: Added `ENGLISH_I_AM_GOING_TO` (`i am going to english` — planned future vs will), `ENGLISH_INFINITIVE_PATTERN` (`the verb pattern english` — to + verb semi-modal frame), `ENGLISH_I_WOULD_LIKE` (`i would like english` — I'd like / Would you like). Total: 13 entries in HAY_UNITS.

**Loop catalog:** JAPANESE_UNITS, KOREAN_UNITS, MANDARIN_UNITS, ENGLISH_UNITS (22 entries each) were already added in prior session. ALL_UNITS merge at line ~5310 includes all 9 languages.

**textbook_lesson_content:** NOT manually seeded for JA/KO/ZH/EN. Confirmed this is auto-generated by the enrichment service on demand (Spanish itself only at 65% coverage — it's never been manually filled). The 8–12% coverage on new languages is normal and will grow as students access lessons.

**Language parity status (all 9 complete):**
- Spanish 1–4: 48/17/8/8 ✓ (master template)
- French 1–4: 48/17/8/8 ✓
- Italian 1–4: 48/17/8/8 ✓
- Portuguese 1–4: 48/17/8/8 ✓
- German 1–4: 48/17/8/8 ✓
- Japanese 1–4: 48/17/8/8 ✓ (loop catalog 22, HayUnitContent 13)
- Korean 1–4: 48/17/8/8 ✓ (loop catalog 22, HayUnitContent 13)
- Mandarin 1–4: 48/17/8/8 ✓ (loop catalog 22, HayUnitContent 13)
- English 1–4: 48/17/8/8 ✓ (loop catalog 22, HayUnitContent 13)

**No blockers. The 9-language mirror project is complete.**
...
**Session: June 18, 2026 (fifth session) — Context caching backup layer via pre-session synthesis**

### What was built / changed

**Context caching workaround for GL Live (`server/services/pre-session-synthesis.ts`)**

Context Caching is confirmed unavailable for GL Live (BidiGenerateContent WebSocket). But the pre-session synthesis step IS a standard REST `generateContent` call — and REST caching fully works there.

David's insight: cache Daniela's static identity on Google's REST API servers. At session start, the synthesis model receives her full identity (core character, teaching philosophy, tools-as-senses, relationship with David, voice characteristics, synthesis task) via cache + the student-specific lite context as the user message. The `[DANIELA_STATE]` inner monologue GL receives is now generated by a model that actually knew who Daniela was, not just who this student is. GL is untouched — it still sends its full 34K system prompt.

**What changed in `pre-session-synthesis.ts`:**
- Added `DANIELA_SYNTHESIS_IDENTITY` constant: **17,561 chars (~4,390 tokens)** — well above Google's 2,048-token minimum for caching. Rich, curated identity block covering: core character, teaching philosophy (Madrigal method + ACTFL), tools as senses (not equipment), voice and tone, relationship with David, the inner monologue task instruction.
- Added `getOrCreateSynthesisCache()`: process-level cache registry (same pattern as `gemini-streaming.ts`). Creates the cache once per 55 minutes, returns `cacheName`, gracefully falls back if Google rejects.
- Updated `generatePreSessionSynthesis()`: tries cached path first (`gemini-2.5-flash` + cached system instruction + lite student context as user message). Falls back to original uncached path (`gemini-3-flash-preview` + inline system instruction) if cache is unavailable or errors. Logs which path was used.
- Model for cached path: `gemini-2.5-flash` (confirmed cache-compatible, no tool conflict because synthesis has no tools).

**What stays the same:** GL session, system prompt assembly, `wrapSynthesisForSystemPrompt`, all calling code in `unified-ws-handler.ts`. No other files changed.

### What's unresolved / Alden should know
- First GL session after server start will create the synthesis cache (one-time ~200ms API call). Second+ sessions within 55 minutes use the cached path (cache hit, cheaper + faster).
- If Google returns a caching error (wrong model, API change, billing), the fallback is silent — sessions continue exactly as before.
- The `DANIELA_SYNTHESIS_IDENTITY` block is intentionally NOT auto-generated from the DB. It's a curated, stable representation of her character. Update it when her fundamental identity evolves (not on every feature add).
- Potential future improvement: cache the actual built system prompt (static sections only) rather than the hand-curated block, to keep them in sync automatically. This would require extracting truly static sections from `createSystemPrompt()` — medium-complexity refactor.
