...
**Immersive Voice & Subtitle System (Beginner-Optimized Architecture)**: The voice chat provides an optimal learning experience for beginners with a strategic audio/visual split: (1) Voice speaks English explanations with authentic Spanish accent - the Spanish TTS voice (es-US-Chirp-HD-O) naturally pronounces both English AND Spanish words, creating immersive accent exposure without overwhelming beginners with complex Spanish they can't understand yet; (2) **Dual-Subtitle System** - When AI provides encouragement (e.g., "¡Perfecto!"), the Spanish encouragement word is **both spoken and shown**: TTS speaks "¡Perfecto!" with authentic pronunciation, followed by the English explanation. Subtitles show a timed sequence: first the encouragement word (2 seconds), then the teaching word/phrase ("Buenos días", "Hola") allowing multi-word phrases at beginner level. Server-side validation detects encouragement words, prepends them to TTS speech, and extracts teaching phrases from the native field, creating subtitle sequences automatically via the `subtitlesJson` database field. The teaching word/phrase **persists on screen after audio finishes**, providing a visual reference for students to practice pronunciation; (3) AI responses use structured JSON format with "target" field (Spanish word/phrase for subtitle) and "native" field (English explanation with Spanish words embedded naturally). Questions ending with "?" are automatically removed and replaced with "Try saying it!"; (4) Full conversation history in the History view shows both languages for reference. This approach ensures beginners hear clear explanations in English while seeing only target language vocabulary, maximizing comprehension and authentic accent exposure.
