...
**Voice Chat Feature**: Real-time voice conversations using OpenAI Realtime API with a WebSocket proxy. Includes manual push-to-talk recording, visual feedback, and a toggle between text and voice modes. Simplified configuration for broader compatibility.
  - **Listen-and-Repeat for Beginners**: In voice mode at beginner difficulty (Phases 1-3), the AI tutor uses structured listen-and-repeat teaching patterns to help students practice pronunciation. The tutor introduces words clearly, says them slowly with natural pauses, and prompts students to repeat. This pedagogical approach is automatically disabled for intermediate and advanced levels, where natural conversational practice is more appropriate.
