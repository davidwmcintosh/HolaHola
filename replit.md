...
Core data models include Users, Conversations, VocabularyWords, and UserProgress. The system features a "Neural Network for Pedagogical Strategies," AI-powered conversation tagging, a Syllabus-Aware Competency System, and centralized Role-Based Access Control (RBAC). Daniela's Wisdom & Relationship Layer includes: `derived_teaching_wisdom` (emergent insights from live teaching), `resonance_anchors` (breakthrough moments that define student relationships), and `relational_temperature` (emotional phase tracking for each student relationship). HolaHola provides pre-built syllabi across 9 languages and a unified ACTFL assessment system. A Voice Diagnostics System provides observability and auto-remediation for TTS degradation.
