...
**Avatar Design**: The tutor avatar uses three distinct, professional Lottie animations (85KB each) for minimal, non-distracting visual feedback. Each state has a unique animation: (1) **Idle** - subtle breathing circle (opacity 0.7, "Ready to help" label, no icon), (2) **Listening** - pulsing circle with Mic icon overlay (opacity 0.9, "Listening to you..." label), (3) **Teaching** - breathing circle with Volume2 icon overlay (opacity 0.95, "Teaching" label). All animations are self-hosted from `attached_assets/lottie/` (`tutor_idle_breathing.json`, `tutor_listening_pulse.json`, `tutor_speaking_breathing.json`) and include clear state labels with ARIA attributes for accessibility. Unique `data-testid` values (`avatar-state-idle`, `avatar-state-listening`, `avatar-state-speaking`) enable automated testing. This design provides subtle motion without distraction while maintaining clear system state feedback across both `ImmersiveTutor.tsx` (full-screen voice chat) and `InstructorAvatar.tsx` (smaller text chat avatar) components.
