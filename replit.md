...
**TTS ACCENT BEHAVIOR (Feb 2026):** Latin-script language tutors (Spanish, French, German, Italian, Portuguese) always use their native language code for TTS to preserve authentic accents even when speaking English. Non-Latin-script languages (Japanese, Korean, Chinese, Hebrew, etc.) override to English TTS when text is 70%+ Latin to ensure intelligibility. Trade-off: accents sound authentic but may cause occasional mispronunciations of English words. Revisit in `server/text-utils.ts` → `detectTextLanguageForTTS()` if mispronunciations become problematic.
