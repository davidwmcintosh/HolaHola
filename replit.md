...
**Avatar Design**: The tutor avatar uses a minimal, non-distracting CSS-animated circular indicator with distinct visual states. When idle, it displays a subtle muted circle. During listening (recording), it shows a pulsing blue circle with a microphone icon. When teaching (speaking), it displays a ping-animated circle with a volume icon. Each state includes clear text labels ("Ready", "Listening", "Teaching") and ARIA labels for accessibility. This design eliminates the distraction of complex animations while maintaining clear system feedback.
