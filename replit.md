...
**Voice Mode Enhancements (November 2025)**: The voice chat experience now includes three key improvements: (1) Auto-play greeting - when starting a new voice conversation, the AI's initial greeting is automatically spoken aloud with race-condition-safe state management using refs to prevent conflicts with active recordings; (2) Navigation shortcuts - "Call Tutor" (sidebar) and "Start Practicing" (hero) buttons now force creation of new conversations like the "New Chat" button, using accessible Link components with localStorage flags; (3) Keyboard accessibility - Ctrl/Cmd+Space shortcut toggles microphone recording with proper guards to prevent hijacking page scroll or input fields, improving hands-free operation.
