...
**Immersive Voice & Subtitle System (Beginner-Optimized Architecture)**: The voice chat provides an optimal learning experience for beginners with a strategic audio/visual split: (1) Voice speaks English explanations with authentic Spanish accent - the Spanish TTS voice (es-US-Chirp-HD-O) naturally pronounces both English AND Spanish words, creating immersive accent exposure without overwhelming beginners with complex Spanish they can't understand yet; (2) **Multi-Phrase Subtitle System** - When AI provides encouragement (e.g., "¡Perfecto!"), the Spanish encouragement word is **both spoken and shown**: TTS speaks "¡Perfecto!" with authentic pronunciation, followed by the English explanation. Subtitles show a timed sequence of ALL Spanish teaching phrases mentioned in the response: encouragement (2 seconds) → phrase 1 (3 seconds) → phrase 2 (3 seconds) → requested phrase (persists). For example, if the AI teaches greetings and says "Try saying 'Buenos días'!", subtitles will show "¡Perfecto!" → "Buenos días" → "buenas tardes" → "buenas noches" → "Buenos días" (persists). Server-side validation detects encouragement words, prepends them to TTS speech, extracts ALL teaching phrases from the native field, and identifies the specific phrase the tutor asks the student to repeat (from "Try saying X" patterns), ensuring that phrase appears last and persists on screen via the `subtitlesJson` database field. The final phrase **persists on screen after audio finishes**, providing a visual reference for what the student should practice; (3) AI responses use structured JSON format with "target" field (Spanish word/phrase for subtitle) and "native" field (English explanation with Spanish words embedded naturally). Questions ending with "?" are automatically removed and replaced with "Try saying it!"; (4) Full conversation history in the History view shows both languages for reference. This approach ensures beginners hear clear explanations in English while seeing ALL target language vocabulary mentioned, maximizing comprehension and authentic accent exposure.
