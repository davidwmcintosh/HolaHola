...
**Neon Database Migration (PHASE 2 - ROUTING VALIDATION)**: Core routing infrastructure complete. Migrated from Replit's isolated dev/prod databases to Neon PostgreSQL with a dual-database hybrid architecture. The SHARED database contains Daniela's intelligence (2.47M agent_observations), curriculum content (138K drill items), Wren insights (138), and Hive collaboration data - accessible by both dev/prod for "One Daniela Everywhere". The USER database (branched) contains per-environment user data: accounts, conversations, sessions, and progress. Table routing in `server/db.ts` uses `getSharedDb()` and `getUserDb()` helper functions that route to Neon when `USE_NEON_ROUTING=true`. The `server/neon-db.ts` file defines `SHARED_TABLES` and `USER_TABLES` sets for classification. Storage layer (`server/storage.ts`) updated with 48+ queries using Neon routing. **Recent fixes (Jan 2026)**: MEMORY_LOOKUP now executes immediately and stores results for multi-step function calls; EXPRESS_LANE_LOOKUP updated to use `collaborationMessages` table with correct `role` column; `syllabus-analytics-service.ts` updated to use `getUserDb()`. **Remaining work**: ~12 services still use direct `db` access - see `docs/neon-routing-audit.md` for prioritized list and Phase 3 exit criteria. **Deprecated**: `sync-bridge.ts` and `sync-scheduler.ts` marked for removal in Phase 3. Migration verification scripts: `scripts/check-neon-full-status.ts`, `scripts/check-all-shared-tables.ts`.
