...
**Karaoke Subtitle Timing Architecture**: The 3-state subtitle system (`subtitleMode` in LanguageContext) uses progressive word-by-word reveal synchronized with audio playback. During playback with word timings, words appear only as spoken (controlled by `visibleWordCount` synced to audio currentTime), with the current word highlighted via `highlightedWordIndex`. When timings are unavailable or after playback ends, the system gracefully degrades to static text display for reading practice. The ImmersiveTutor component handles all rendering: progressive mode during playback with timings, static fallback for playback without timings or post-playback review. The VoiceChatViewManager provides view switching (live/history) and subtitle mode cycling (Off → Target → All → Off).
