...
**Immersive Voice & Subtitle System (Beginner-Optimized Architecture)**: The voice chat provides an optimal learning experience for beginners with a strategic audio/visual split: (1) Voice speaks English explanations with authentic Spanish accent - the Spanish TTS voice (es-US-Chirp-HD-O) naturally pronounces both English AND Spanish words, creating immersive accent exposure without overwhelming beginners with complex Spanish they can't understand yet; (2) On-screen subtitles display ONLY the target language word being taught ("Hola", "Gracias") - keeping visual focus on the new vocabulary; (3) AI responses use structured JSON format with "target" field (Spanish word only) and "native" field (English explanation with Spanish words embedded naturally); (4) Full conversation history in the History view shows both languages for reference. This approach ensures beginners hear clear explanations in English while seeing only target language vocabulary, maximizing comprehension and authentic accent exposure.
