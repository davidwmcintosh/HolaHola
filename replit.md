...
**Neon Database Migration (PHASE 3 COMPLETE ✅)**: Migrated from Replit's isolated dev/prod databases to Neon PostgreSQL with a dual-database hybrid architecture. The SHARED database contains Daniela's intelligence (2.47M agent_observations), curriculum content (138K drill items), Wren insights (138), and Hive collaboration data - accessible by both dev/prod for "One Daniela Everywhere". The USER database (branched) contains per-environment user data: accounts, conversations, sessions, and progress. Table routing in `server/db.ts` uses `getSharedDb()` and `getUserDb()` helper functions that route to Neon when `USE_NEON_ROUTING=true`. The `server/neon-db.ts` file defines `SHARED_TABLES` and `USER_TABLES` sets for classification. Storage layer (`server/storage.ts`) and all 26+ services now use proper Neon routing. **Phase 2 completion (Jan 2026)**: All services migrated to Neon routing. MEMORY_LOOKUP and EXPRESS_LANE_LOOKUP functions execute immediately. **Phase 3 completion (Jan 2026)**: Removed 8,700+ lines deprecated code (sync-bridge.ts, sync-scheduler.ts, editor-persona-service.ts). Sync API routes return 410 Gone with migration guidance. Runtime confirms: "[CONSOLIDATION] Sync-bridge retired - Neon routing is primary". **Phase 4 (Next)**: Remove USE_NEON_ROUTING flag after 2+ week soak period. See `docs/neon-routing-audit.md` for details. Migration verification: `scripts/check-all-shared-tables.ts` (36/36 tables verified).
