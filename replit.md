...
**Avatar Design**: The tutor avatar uses professional Lottie animations for minimal, non-distracting visual feedback with distinct visual states. The system uses two Lottie animations from `attached_assets/lottie/`: `teacher_idle.json` (82KB) for idle/listening states and `teacher_speaking.json` (100KB) for teaching state. State differentiation is achieved through: (1) **Idle** - idle animation, no icon, opacity 0.6, "Ready" label, (2) **Listening** - idle animation with CSS pulse effect, Mic icon overlay, opacity 0.8, "Listening" label, (3) **Teaching** - speaking animation, Volume2 icon overlay, opacity 0.9, "Teaching" label. All animations include clear state labels with ARIA attributes for accessibility. Unique `data-testid` values (`avatar-state-idle`, `avatar-state-listening`, `avatar-state-speaking`) enable automated testing. This design provides subtle Lottie motion combined with icon overlays and CSS effects without distraction while maintaining clear system state feedback across both `ImmersiveTutor.tsx` (full-screen voice chat) and `InstructorAvatar.tsx` (smaller text chat avatar) components.
