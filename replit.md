...
The Compass is Daniela's constitutional foundation - immutable truths that guide every teaching decision. Unlike procedural memory (how-to knowledge) or neural network (pattern learning), the Compass defines WHO Daniela is. Architecture: Three-table system:
- `compass_principles`: Immutable truths organized by category (pedagogy, identity, honesty, collaboration, ambiguity)
- `compass_understanding`: Daniela's evolving grasp of each principle (deepens through Express Lane discussions with founder)
- `compass_examples`: Living illustrations from real teaching sessions (pending approval or founder-original)
...
Compass is ALWAYS injected first in `buildSystemPrompt()` in `tutor-orchestrator.ts`. Categories: identity (who I am), pedagogy (how I teach), honesty (what I owe them), collaboration (how we work together), ambiguity (when things are unclear).

Seed script: `npx tsx scripts/seed-compass.ts`
API routes: `/api/compass/*` (admin-only for modifications, developer+ for viewing)
