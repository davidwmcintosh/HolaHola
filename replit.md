...
**Neon Database Migration (COMPLETE)**: Successfully migrated from Replit's isolated dev/prod databases to Neon PostgreSQL with a dual-database hybrid architecture. The SHARED database contains Daniela's intelligence (2.47M agent_observations), curriculum content (138K drill items), Wren insights (138), and Hive collaboration data - accessible by both dev/prod for "One Daniela Everywhere". The USER database (branched) contains per-environment user data: accounts, conversations, sessions, and progress. Table routing in `server/db.ts` uses `getSharedDb()` and `getUserDb()` helper functions that route to Neon when `USE_NEON_ROUTING=true`. The `server/neon-db.ts` file defines `SHARED_TABLES` and `USER_TABLES` sets for classification. Storage layer (`server/storage.ts`) updated to use Neon for observation queries. This replaces the 8,000-line sync-bridge system with database-native reliability. Migration verification scripts: `scripts/check-neon-full-status.ts`, `scripts/check-all-shared-tables.ts`.
