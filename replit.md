...
**TTS LANGUAGE SWITCHING (Feb 2026):** Word-by-word language segmentation is ACTIVE via `language-segmenter.ts`. Mixed sentences (e.g., "Try saying hermoso") get segmented so target language words use target TTS and native language words use native TTS. The fallback `detectTextLanguageForTTS()` in `server/text-utils.ts` detects language via diacritics/script - Latin text without target language markers defaults to English TTS for clear pronunciation. This approach scales to any native language (Italian learner gets clean Italian for explanations, proper Spanish for vocab). Previously tried "accent mode" (always use target language TTS for authenticity) but reverted because it would cause accent bleed when native language isn't English.
