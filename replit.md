...
Wren's intelligence is enhanced by the Wren Intelligence Service for insight capture and the Wren Proactive Intelligence Service, which includes Proactive Triggers, a Daniela Feedback Loop, Architectural Decision Records (ADR), a Priority Inference Engine, and Project Health Awareness. **Wren's Architectural Memory** is initialized at server startup via `initReplitMdCache()` in hive-consciousness-service.ts, which async-loads and parses replit.md sections (Overview, System Architecture, External Dependencies). The `getWrenArchitecturalContext()` function combines this cached baseline with max 2 parallel database queries (combined keyword search + architecture insights) to build Wren's contextual memory for each response. Future contributors should call `initReplitMdCache()` during startup rather than re-reading the file synchronously. The Student Learning Service tracks student error patterns, validates teaching strategies, and injects personalized learning context into Daniela's prompts.
