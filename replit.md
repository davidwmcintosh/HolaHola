...
**Neon Database Migration (PHASE 2 COMPLETE ✅)**: Migrated from Replit's isolated dev/prod databases to Neon PostgreSQL with a dual-database hybrid architecture. The SHARED database contains Daniela's intelligence (2.47M agent_observations), curriculum content (138K drill items), Wren insights (138), and Hive collaboration data - accessible by both dev/prod for "One Daniela Everywhere". The USER database (branched) contains per-environment user data: accounts, conversations, sessions, and progress. Table routing in `server/db.ts` uses `getSharedDb()` and `getUserDb()` helper functions that route to Neon when `USE_NEON_ROUTING=true`. The `server/neon-db.ts` file defines `SHARED_TABLES` and `USER_TABLES` sets for classification. Storage layer (`server/storage.ts`) and all 26+ services now use proper Neon routing. **Phase 2 completion (Jan 2026)**: All services in `server/services/*.ts`, `server/routes.ts`, and `server/reporting-service.ts` now use proper `getUserDb()`/`getSharedDb()` routing. MEMORY_LOOKUP and EXPRESS_LANE_LOOKUP functions execute immediately and store results for multi-step function calls. **Phase 3 (Next)**: Remove deprecated sync-bridge.ts, sync-scheduler.ts, editor-persona-service.ts. See `docs/neon-routing-audit.md` for Phase 3 exit criteria. Migration verification scripts: `scripts/check-neon-full-status.ts`, `scripts/check-all-shared-tables.ts`.
