...
A Dev-Prod Sync System synchronizes critical data batches between environments, supporting bidirectional sync and delta syncing for efficiency. **Peer Wake-Up (v36)**: Before sync operations, the system pings the peer environment's health endpoint to wake it from Replit's sleep mode. If the peer is already awake, sync proceeds immediately; if sleeping, exponential backoff retries (10s, 20s, 40s) with a 5s initialization delay after wake. **Resumable Sync (v37)**: Enhanced resume logic now detects failed/partial syncs (not just stuck 'running' syncs) within 10 minutes that have progress (completedBatches or lastCompletedPage). The Sync Control Center shows a prominent "Resume" button when a resumable sync is detected, displaying progress info. Auto-resume triggers for recent failures with progress. **Paginated Curriculum Drills (v39)**: The curriculum-drills batch is now paginated (500 drills per page) to handle 98K+ drill items without 413 Request Entity Too Large errors. A new `actfl-core` batch separates ACTFL tables (grammarCompetencies, canDoStatements, lessonCanDoStatements, culturalTips) from drills for reliable sync. Pagination uses consistent naming (curriculum-drills-p0, curriculum-drills-p1, etc.) with resume support via `lastCompletedPage`. **Delta Sync All (v40)**: All curriculum tables now have `updatedAt` timestamps enabling delta sync filtering (only sync records changed since last successful push). This significantly speeds up subsequent syncs by only transferring modified records instead of the full 98K+ dataset. An Observation Summarization System condenses observations into insights using Gemini. The Daniela Content Growth System enables autonomous pedagogical content creation.
