/**
 * test-prequel-episode-2-db-sync.ts
 *
 * CI check: confirms that docs/prequel-episode-2.md and the DB record
 * conversation_memories id = db89e9df-c0ba-4eb8-9004-df398f3237d9 contain
 * the same content (modulo trailing whitespace normalization).
 *
 * If the .md has been edited but the DB was not updated via
 * sync-prequel2-content.ts, this check fails loudly.
 *
 * Run: npx tsx server/scripts/test-prequel-episode-2-db-sync.ts
 */
...
const EPISODE_ID = 'db89e9df-c0ba-4eb8-9004-df398f3237d9';
const MD_PATH = join(process.cwd(), 'docs', 'prequel-episode-2.md');
...
async function main() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  sep();
  console.log(B('PART 1 — Read docs/prequel-episode-2.md'));
  sep();
  ...
  if (!inSync) {
    ...
    console.log(R(`\n  FIX: run  npx tsx server/scripts/sync-prequel2-content.ts\n`));
  }
  ...
  assert(
    ...
    inSync
      ...
      : 'Content diverged — the .md was edited without syncing the DB. ' +
        'Run: npx tsx server/scripts/sync-prequel2-content.ts',
  );
  ...
  if (failed === 0) {
    ...
    console.log(G('   docs/prequel-episode-2.md and DB record db89e9df are in sync.\n'));
    process.exit(0);
  } else {
