/**
 * test-prequel-episode-4-db-sync.ts
 *
 * CI check: confirms that docs/prequel-episode-4.md and the DB record
 * conversation_memories id = ce9a2f9e-8b36-4723-9a16-f878b7035aa9 contain
 * the same content (modulo trailing whitespace normalization).
 *
 * If the .md has been edited but the DB was not updated via
 * sync-prequel-episode-4.ts, this check fails loudly.
 *
 * Run: npx tsx server/scripts/test-prequel-episode-4-db-sync.ts
 */
...
async function main() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  sep();
  console.log(B('PART 1 — Read docs/prequel-episode-4.md'));
  sep();
  ...
  if (!inSync) {
    ...
    console.log(R(`\n  FIX: run  npx tsx server/scripts/sync-prequel-episode-4.ts\n`));
  }
  ...
  assert(
    ...
    inSync
      ...
      : 'Content diverged — the .md was edited without syncing the DB. ' +
        'Run: npx tsx server/scripts/sync-prequel-episode-4.ts',
  );
  ...
  if (failed === 0) {
    ...
    console.log(G('   docs/prequel-episode-4.md and DB record ce9a2f9e are in sync.\n'));
    process.exit(0);
  } else {
