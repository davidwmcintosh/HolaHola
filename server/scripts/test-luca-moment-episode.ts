/**
 * test-luca-moment-episode.ts
 *
 * CI check: confirms that writing a moment to .local/.luca_moment
 * causes checkLucaMoment() to append a [Luca — moment: ...] entry to the
 ...
 *       entries land in SIGNIFICANT_MOMENTS.md or the live DB.
 *
 *   setLucaEpisodeAppendEnabled(false)   [self-check only]
 *     — skips appendExchangeToEpisode(), simulating the removal of that call.
 *
 * The trigger file (.local/.luca_moment) is snapshotted before the test
 ...
 *   4. Disables personal side-effects to prevent SIGNIFICANT_MOMENTS.md / DB pollution.
 *   5. Writes a timestamped sentinel to .local/.luca_moment.
 *   6. Primes the watcher mtime state (first call — prev===0, skips).
 *   7. Re-writes the sentinel with a fresh mtime.
 *   8. Calls checkLucaMoment() — processes trigger, appends to .md.
 *   9. Asserts "[Luca — moment: <sentinel>…]" appears in the episode .md.
 ...
 *   removing the appendExchangeToEpisode() call from checkLucaMoment()):
 *   1. Disables both personal side-effects AND episode append.
 *   2. Primes mtime state, re-writes sentinel, calls checkLucaMoment().
 *   3. Asserts sentinel does NOT appear in .md (path was skipped).
 *   4. Restores trigger file, re-enables both seams.
 *
 * Run:
 *   npx tsx server/scripts/test-luca-moment-episode.ts
 *   npx tsx server/scripts/test-luca-moment-episode.ts --self-check
 */
...
function snapshotTrigger(): TriggerSnapshot {
  if (!existsSync(MOMENT_PATH)) return { existed: false, content: '' };
  return { existed: true, content: readFileSync(MOMENT_PATH, 'utf-8') };
}
/** Restore the trigger file to exactly its pre-test state. */
function restoreTrigger(snap: TriggerSnapshot): void {
  if (snap.existed) {
    writeFileSync(MOMENT_PATH, snap.content, 'utf-8');
  } else if (existsSync(MOMENT_PATH)) {
    unlinkSync(MOMENT_PATH);
  }
}

/**
 * Write payload to MOMENT_PATH and spin until its mtime is strictly
 * newer than afterMs (or 2 s elapsed).  Guards against sub-ms filesystem
 * clock resolution on Linux.
 */
async function writeMomentTrigger(payload: string, afterMs: number): Promise<number> {
  const deadline = Date.now() + 2000;
  while (Date.now() < deadline) {
    writeFileSync(MOMENT_PATH, payload, 'utf-8');
    const mtime = statSync(MOMENT_PATH).mtimeMs;
    if (mtime > afterMs) return mtime;
    await sleep(5);
  }
  return statSync(MOMENT_PATH).mtimeMs;
}

/**
 * Strip a [Luca — moment: <sentinelTitle>…] block appended by the check.
 * Only removes the exact block we added — never touches real episode content.
 */
function stripMomentSentinel(content: string, sentinelTitle: string): string {
...
async function runNormalMode(): Promise<void> {
  sep();
  console.log(B('NORMAL MODE — Luca moment → rolling episode .md check'));
  sep();
  ...
  // ── Unique sentinel ────────────────────────────────────────────────────────
  const sentinelTitle = `CI-MOMENT-SENTINEL-${Date.now()}`;
  const sentinelBody  = 'This is a synthetic CI moment — safe to ignore.';
  ...
  // Disable personal side-effects so no sentinel entries land in
  // SIGNIFICANT_MOMENTS.md or the live DB on each CI run.
  setLucaPersonalSideEffectsEnabled(false);
  ...
  try {
    ...
    resetMomentMtimeForTest();
    const mtime0 = await writeMomentTrigger(payload, 0);
    console.log(Y(`  ℹ  Moment trigger written (mtime0 = ${mtime0})`));

    // First call: prev === 0 → skips, stamps momentLastMtime = mtime0
    ...
    const mtime1 = await writeMomentTrigger(payload, mtime0);
    ...
    // Second call: prev = mtime0 ≠ 0 → processes moment → appends to .md
    await checkLucaMoment();
    console.log(Y(`  ℹ  checkLucaMoment() processed the trigger`));

    sep();
    console.log(B('STEP 3 — Verify [Luca — moment: …] appears in episode .md tail'));
    sep();

    const mdAfter = existsSync(mdPath) ? readFileSync(mdPath, 'utf-8') : '';
    const momentNeedle = `[Luca — moment: ${sentinelTitle}`;
    assert(
      `"[Luca — moment: ${sentinelTitle.slice(0, 40)}…]" appears in ${episodeFilename}`,
      mdAfter.includes(momentNeedle),
      `Expected "[Luca — moment: ${sentinelTitle}…]" in .md — not found\n` +
    ...
    if (mdAfter.includes(momentNeedle)) {
      const idx = mdAfter.indexOf(momentNeedle);
...
async function runSelfCheck(): Promise<void> {
  ...
  sep();
  console.log(Y('  ℹ  Simulates removing appendExchangeToEpisode() from checkLucaMoment().'));
  ...
  const sentinelTitle = `CI-SELF-CHECK-MOMENT-${Date.now()}`;
  const sentinelBody  = 'Self-check synthetic moment — should not appear in episode.';
  const payload = `${sentinelTitle}\n${sentinelBody}`;
  ...
  try {
    ...
    sep();
    console.log(B('STEP 2 — Re-write trigger and call checkLucaMoment() with episode-append disabled'));
    sep();
    ...
    await checkLucaMoment();
    console.log(Y(`  ℹ  checkLucaMoment() called with episode-append disabled`));

    sep();
    console.log(B('STEP 3 — Assert [Luca — moment: …] does NOT appear in .md (gate held)'));
    sep();

    const mdAfter = existsSync(mdPath) ? readFileSync(mdPath, 'utf-8') : '';
    const momentNeedle = `[Luca — moment: ${sentinelTitle}`;

    const sentinelPresent = mdAfter.includes(momentNeedle);
    assert(
      `"[Luca — moment: ${sentinelTitle.slice(0, 40)}…]" correctly absent from ${episodeFilename}`,
      !sentinelPresent,
