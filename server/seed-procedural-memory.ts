...
async function seedToolKnowledge() {
  const tools = [
    ...
    {
      toolName: 'PRONUNCIATION_TAG',
      toolType: 'voice_control',
      purpose: 'Mark a word for correct native pronunciation when speaking. Use [lang:word] to ensure a word is pronounced with the correct language phonetics. Your voice is bilingual - you can speak English naturally but pronounce target language words authentically without switching voices.',
      syntax: '[lang:word] where lang is a 2-letter code (es, fr, de, it, pt, ja, ko, zh, he) or full name (spanish, french, german, etc.)',
      examples: [
        'The word [es:casa] means house.',
        'Try saying [es:buenos días] when you greet someone.',
        'Compare [es:gracias] in Spanish with [fr:merci] in French.',
        'The verb [es:ser] is for permanent qualities, while [es:estar] is for temporary states.',
        'In Hebrew, [he:שָׁלוֹם] means peace.',
      ],
      bestUsedFor: ['teaching_vocabulary', 'mixed_language_explanations', 'pronunciation_modeling', 'comparing_languages', 'introducing_new_words'],
      avoidWhen: ['speaking_entirely_in_target_language'],
      combinesWith: ['WRITE', 'PHONETIC'],
      sequencePatterns: ['Say [es:word] with tag → student hears correct pronunciation → WRITE the word → student practices'],
      notes: 'This is your paintbrush for pronunciation. When explaining in English but introducing vocabulary, tag the vocabulary words so they sound authentic. You are one bilingual speaker - the tag just ensures phonetic accuracy without changing your voice or personality. SAFE TO USE ALWAYS: If a word is not in the phoneme dictionary, it simply passes through unchanged - so tag liberally! Better to tag and let the system handle it than to miss pronunciation opportunities.',
    },
    {
      toolName: 'COMPARE',
      toolType: 'whiteboard_command',
    ...
    // VOICE_ADJUST - Real-time voice expressiveness control
    {
      toolName: 'VOICE_ADJUST',
      toolType: 'voice_control',
      purpose: 'OUTPUT THE LITERAL TAG [VOICE_ADJUST speed="..." emotion="..." personality="..."] to modify your voice in real-time. Changes PERSIST until you use [VOICE_RESET]. Use for teaching moments - slow down for pronunciation, speed up for excitement, change personality for different teaching styles.',
      syntax: '[VOICE_ADJUST speed="slowest|slow|normal|fast|fastest" emotion="happy|excited|friendly|curious|warm|playful|surprised|proud|encouraging|calm" personality="warm|calm|energetic|professional"]',
      examples: [
        ...
        '"[VOICE_ADJUST personality="calm" emotion="patient"] Lets slow down and take this step by step."',
        '"[VOICE_ADJUST personality="energetic" speed="fast"] This is so exciting - you are doing amazing!"',
      ],
      bestUsedFor: ['celebration_moments', 'emphasis_on_pronunciation', 'building_excitement', 'calming_frustrated_student', 'dramatic_teaching_moments', 'slowing_for_difficult_content', 'adapting_to_student_mood'],
      avoidWhen: ['overusing_constantly', 'student_prefers_calm_consistent_tone', 'technical_explanations'],
      combinesWith: ['WRITE', 'DRILL', 'PHONETIC', 'VOICE_RESET'],
      sequencePatterns: [
        'Student succeeds → [VOICE_ADJUST emotion="excited"] → enthusiastic praise → [VOICE_RESET]',
        'Difficult pronunciation → [VOICE_ADJUST speed="slowest"] → clear slow pronunciation → [VOICE_RESET]',
        'Student frustrated → [VOICE_ADJUST personality="calm" speed="slow"] → calm reassurance → [VOICE_RESET]',
        'Excitement moment → [VOICE_ADJUST personality="energetic" emotion="excited"] → celebrate → [VOICE_RESET]',
      ],
    },
    
    // VOICE_RESET - Return to tutor's baseline voice settings
    {
      toolName: 'VOICE_RESET',
      toolType: 'voice_control',
      purpose: 'OUTPUT THE LITERAL TAG [VOICE_RESET] to return to your baseline voice settings. Use after special teaching moments to restore your natural voice. Your defaults are saved when the session starts.',
      syntax: '[VOICE_RESET] or [VOICE_RESET reason="..."]',
      examples: [
        '"[VOICE_RESET] Okay, lets continue with our lesson."',
        '"[VOICE_RESET reason="returning to normal pace"] Now that weve practiced that, lets move on."',
        '"Great job with that pronunciation! [VOICE_RESET] What would you like to work on next?"',
      ],
      bestUsedFor: ['after_celebration', 'after_slow_pronunciation_practice', 'transitioning_between_activities', 'returning_to_normal_teaching'],
      avoidWhen: ['mid_special_moment', 'student_still_needs_adjusted_voice'],
      combinesWith: ['VOICE_ADJUST'],
      sequencePatterns: [
        '[VOICE_ADJUST ...] → special teaching moment → [VOICE_RESET]',
        'Slow pronunciation practice → [VOICE_RESET] → continue lesson at normal pace',
        'Celebrate success → [VOICE_RESET] → transition to next activity',
      ],
    },
  ];
