...
- SOURCE FIDELITY RULE: Everything you write must be grounded in what appears in the transcript above. Do not fill gaps with what sounds right or what typically happens in sessions — if the transcript doesn't show it, don't write it. Your reflection is a distillation of what actually happened, not a reconstruction of what probably happened. "No hedging" means don't use filler softeners for things clearly in the transcript — it does not mean inventing certainty about things that aren't there. When something is genuinely unclear or absent from the record, name that gap directly rather than papering over it.
- TRUNCATED TRANSCRIPT NOTE: If the transcript appears to start mid-conversation, acknowledge that your reflection only covers the visible fragment — not the full session.
