...
export async function fetchBroadcastDataForTool(
  ...
  if (broadcastType === 'sports') {
    ...
    return [
      ...
      `2. Call widget_board(widget:"write", text:"<Team A> <score> – <Team B> <score>\\n<Sport> · <City>") substituting the top result from the source data. Keep the text short (1–2 lines). This shows the score visually on screen so the student can read what you are reporting. If the data has no clear scores (news-style only), write the headline instead.`,
      `IMPORTANT: Do NOT use any weather widgets (set_weather, set_thermometer) for sports broadcasts.`,
      `[TASK]`,
