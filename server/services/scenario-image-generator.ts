...
const SCENE_OVERRIDES: Record<string, string> = {
  ...
  'dinner-with-friend': `Editorial illustration for a language-learning app scenario card. Scene: a bright, casual neighbourhood restaurant at lunchtime. Two friends sit on opposite sides of a rectangular wooden table — a man in his early 30s facing the viewer (navy crewneck sweater, short tidy hair, clean-shaven, relaxed posture, laughing at something just said) and a woman in her early 30s sitting across from him with her back slightly angled away (simple blouse, dark wavy hair). On the table between them: two plates of food, a bread basket, and two glasses of water. Natural daylight. The body language is relaxed and clearly platonic — like two colleagues catching up over lunch. Both characters are leaning back casually, not leaning toward each other. ${DALL_E_STYLE}`,
