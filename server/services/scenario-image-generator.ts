...
const SCENE_OVERRIDES: Record<string, string> = {
  ...
  'job-interview': `Office job interview. At the extreme left edge of the image: a tall floor-to-ceiling bookshelf packed with binders and books. At the extreme right edge of the image: a large floor-to-ceiling window with bright daylight streaming in. The entire back wall between them is a warm grey painted surface. In the center foreground, a wooden desk: a female interviewer with a short bob haircut (uncovered, no hat) in a navy blazer sits on one side holding a clipboard; a male candidate with short neat hair (uncovered) in a button-up shirt sits across the desk leaning forward. A small potted succulent on the desk. The floor carpet is visible at the bottom edge. ${DALL_E_STYLE}`,
  ...
  'restaurant': `A warm, inviting local restaurant interior, dinner service. A male waiter in a casual apron stands tableside, notepad in hand, taking an order from a female diner who looks up from her menu with a smile. A lit candle and simple table setting on the table. Other tables blurred softly in the background. Relaxed neighborhood restaurant feel. ${DALL_E_STYLE}`,
...
function buildScenarioPrompt(slug: string, title: string, description: string, location: string | null, category: string, topics: string[]): string {
  ...
  return `Scene for a language-learning app: "${title}" — ${description}${locationHint}${topicHint ? ` Topics: ${topicHint}.` : ''} Two people in conversation in a clearly recognisable setting. ${DALL_E_STYLE}`;
}
