...
async function disableFKConstraints(pool: Pool, tableName: string): Promise<string[]> {
...
async function migrateTableWithCopy(
  ...
  try {
    ...
    if (disableFK) {
      await disableFKConstraints(targetPool, tableName);
    }
    ...
    while (offset < totalRows) {
      const dataResult = await sourcePool.query(
        `SELECT ${columnList} FROM "${tableName}" ORDER BY 1 LIMIT ${BATCH_SIZE} OFFSET ${offset}`
      );
      ...
      for (const row of dataResult.rows) {
        ...
        try {
          await targetPool.query(
            `INSERT INTO "${tableName}" (${columnList}) VALUES (${placeholders.join(', ')}) ON CONFLICT DO NOTHING`,
            values
          );
          migratedRows++;
          ...
          if (!insertError.message.includes('duplicate key') && 
            ...
            console.log(`    Row error: ${insertError.message.substring(0, 80)}`);
          }
        }
      }
      ...
      if (totalRows > BATCH_SIZE) {
        ...
        process.stdout.write(`\r    Progress: ${Math.min(progress, 100)}% (${Math.min(offset, totalRows).toLocaleString()}/${totalRows.toLocaleString()})`);
      }
    }
...
async function runMigration() {
  ...
  const specificTable = process.argv[2];
  const targetDb = process.argv[3];

  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║           NEON DATABASE MIGRATION V2                         ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  if (specificTable) {
    console.log(`Migrating single table: ${specificTable} to ${targetDb || 'AUTO'}\n`);
    const pool = targetDb === 'user' ? userPool : sharedPool;
    const result = await migrateTableWithCopy(sourcePool, pool, specificTable, targetDb || 'SHARED', true);
  ...
  for (const table of SHARED_TABLES) {
    if (existingTables.has(table)) {
      const result = await migrateTableWithCopy(sourcePool, sharedPool, table, 'SHARED', true);
  ...
  for (const table of USER_TABLES) {
    if (existingTables.has(table)) {
      const result = await migrateTableWithCopy(sourcePool, userPool, table, 'USER', true);
  ...
  if (sharedFailed.length > 0) {
    console.log(`  ✗ Failed: ${sharedFailed.map(r => r.table).join(', ')}`);
  }
  ...
  if (userFailed.length > 0) {
    console.log(`  ✗ Failed: ${userFailed.map(r => r.table).join(', ')}`);
  }
